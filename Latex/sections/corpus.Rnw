...
\begin{enumerate}
  \item{\label{continf}  bare infinitives typically as dependents of modals and some other verbs (see example \ref{ex:disjunctspelling34}),}
  \item{\label{contpart} participles as dependents of auxiliaries (see example \ref{ex:disjunctspelling3}),}
  \item{\label{contprog} the so-called progressive \roland{REFERENCE!}(see example \ref{ex:disjunctspelling5}),}
  \item{\label{contnp}   full NPs (see example \ref{ex:disjunctspelling7}).}
\end{enumerate}

\vspace{\baselineskip}

The constructions with the bare infinitive (\ref{continf}) and the participle (\ref{contpart}) represent two prototypically syntactic constructions as the verb from the N+V is part of a verbal syntagma with auxiliaries in the broad sense (including modals etc.).
The NP context (\ref{contnp}) is most prototypically nominal, especially since we only used NPs with a determiner.
More precisely, we only searched the corpus for NPs with definite determiners cliticised to a preposition (\textit{beim} `at the', \textit{zum} `to the', etc.).
This decision was made in order to allow for a comparison of these full nominalisations and the so-called progressive (\ref{contprog}).
...
While it developed out of a construction with a copula and a plain NP and is formally identical to the NPs in (\ref{contnp}), it is often assumed to be a verbal construction expressing progressive meaning.
Including NPs in this specific form along this emerging progressive construction allows us to assess whether the hypothesised verbal semantics of the progressive makes the construction more verbal, leading to a weaker tendency towards univerbations compared to regular NPs.

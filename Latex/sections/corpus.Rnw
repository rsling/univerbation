...
\footnote{One reviewer pointed out -- citing \citet{Gries2022} -- that log odds ratios could be used instead of Cramér's $v$ for the reason that Cramér's $v$ does in many situations not go up to 1 and might have some other undesirable mathematical properties.

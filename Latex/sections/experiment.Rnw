...
It should be kept in mind that the v scores have no interpretation independently of their specific distribution.
...
\footnote{\hl{This is the maximal random effect structure that converges and results in a healthy variance-covariance matrix, see \mbox{\citet{Schaefer2020a}}.}}

...
Then, we discuss previous analyses of N+V units and their spelling in Section~\ref{sec:thestatusofnounverbunitsingerman} and a formulation of predictions for the empirical studies in Sections~\ref{sec:corpusbasedanalysisoftheusageofnvunits} and~\ref{sec:elicitedproductionofnounverbunitsinwrittenlanguage}.

\subsection{Spaces, words, and univerbation}
\label{sub:spaceswordsanduniverbation}

As explained in Section~\ref{sec:introduction}, we use graphemic evidence from corpora and controlled experiments, and we argue that it allows us to draw conclusions about writers' cognitive grammars.
...
Until such a diachronic process is complete and one of the spellings has become clearly dominant, lexicalisation does not provide a very strong input to writers, and they alternate between a syntagmatic and a morphological realisation.
...
However, the decodable relations (argument or oblique) are prototypically realised syntactically as German is a language with a very weak (if any) tendency towards noun incorporation (see below in this section).
...
N+V units are usually seen as the only cases of potential incorporation in Modern German \parencite[245]{Eisenberg2020a}, which is why we postulated above that argument and oblique relations are prototypically realised syntactically in the form of objects and adjuncts.

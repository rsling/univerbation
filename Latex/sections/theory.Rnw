...
To this end, we will shed some light \hl{on particle verbs as a target class for N+V units in Section~\mbox{\ref{sub:particleverbs}}}, \hl{on N+V units as structures involving incorporation in Section~\mbox{\ref{sub:incorporation}}}, \hl{before turning to N+V units as reluctant compounds in Section~\mbox{\ref{sub:nncompounds}}}.
...
\subsubsection{\hl{Particle verbs}}
...
German has a class of verbs with separable prefixes called particle verbs (distinct from verbs with non-separable prefixes called prefix verbs), which obviously serves as such a prototype for N+V units.
...
He views particle verbs as providing a pattern towards which N+V units gravitate when they become a single word.
...
While back-formation might indeed be a factor furthering univerbation, it is virtually impossible to decide for all N+V units currently in use (over 800 in our study, see Section~\ref{sub:choiceofcorpussamplingandannotation}) with good certainty whether they are derived via back-formation or not.%
...
See Section~\ref{sec:explainingnounverbuniverbation} for a further brief discussion.%
}\Up{,}\footnote{%
...
We find that this classification -- derived from Wurzel's intuition and older normative dictionaries and grammars -- is not supported empirically, as many of the allegedly non-existing forms can be found in corpora and even dictionaries and online databases.
...
\subsubsection{\hl{Incorporation and the internal semantic relation}}
...
However, since the oblique relation requires an additional marker (a preposition) when the N+V unit is realised in syntax, the variant with full incorporation loses its direct (almost verbatim) syntactic competitor.
...
On the other hand, \textit{probefahren} -- even spelled as \textit{Probe fahren} -- does not have a syntactic interpretation at all due to the lack of the preposition marking the oblique relation, and hence full incorporation is the only available interpretation.
...
\subsubsection{\hl{N+N compounds}}
...
Hence, we propose that there is an additional prototype that attracts N+V units, namely N+N compounds, at least when under strong syntactic pressure.
...
This particle is, however, virtually part of the verbal word form and does by no means prevent univerbation (see example \ref{ex:compoundspelling4}, where \textit{rad\-zu\-fahren} is spelled as one word instead of \textit{rad zu fahren}).
...
However, an N+V unit does no function as the verbal head in another N+V unit under normal circumstances (\ie a [N+[N+V]] structure) as illustrated in (\ref{ex:nnv}).
...
\subsubsection{\hl{Conclusions for the empirical studies}}
...
First, when the unit occurs in a strongly nominal syntagma (\egg when the head is a fully nominalised head of an NP with a determiner), we expect a high tendency towards univerbation due to a highly accessible N+N compound prototype (Section~{sub:nncompounds}).
...
The other important cue is the internal semantic relation (Section~{sub:incorporation}).
...
N+V units with an oblique relation would need more explicit marking with a preposition in the unambiguously syntactic realisation, and incorporation is the better option than a syntactic structure.

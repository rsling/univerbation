...
In this section, we lay the theoretical and descriptive foundations of our analysis of the alternation described in Section~\ref{sec:introduction}.
First, we clarify the status of spaces (\ie \textit{spatia}) as syntactic boundaries in German in Section~\ref{sub:spaceswordsanduniverbation}.
Then, we discuss previous analyses of N+V units and their spelling, followed by the formulation of our predictions for the empirical studies in Section~\ref{sec:thestatusofnounverbunitsingerman}.

\subsection{Spaces, words, and univerbation}
\label{sub:spaceswordsanduniverbation}

As explained in Section~\ref{sec:introduction}, we use graphemic evidence from corpora and controlled experiments, and we argue that it indirectly allows us to draw conclusions about writers' cognitive grammars.
...
If many speakers consistently make this choice over a significant period of time, the unit might become lexicalised  as a single word \parencite[7]{Lehmann2021}.
Until such a diachronic process is complete and one of the spellings has become clearly dominant, lexicalisation does not provide a very strong input to writers, and the item alternates between a syntactic and a morphological realisation.
...
To this end, we need to shed some light on the productive N+N compound construction in Section~\ref{sub:nncompounds} before turning to N+V units as reluctant compounds in Section~\ref{sub:nvunitsasreluctantcompounds}.
...
While in principle the morphological N+V construction (as a kind of compound written as one word) has existed for centuries, we show why and how it remains in competition with a syntactic construction because it does not fit the morphological compounding prototype very well.
At the same time, we argue why morphological compounding and consequently the spelling as one word (univerbation) become the preferred realisation under the right circumstances.

Most likely, full compounding of N+V units requires conversion of the verbal head to a noun, making the result an N+N compound via conversion.
...
A major difference to N+N compounds is the fact that N+V units are usually not inseparable as was already shown in Section~\ref{sec:introduction}.
...
This particle is, however, virtually part of the verbal word form and does by no means prevent univerbation (see example \ref{ex:compoundspelling4}).
Furthermore, the noun and the verb can be reordered in verb-second order, where N+V units resemble particle verbs \parencite[603]{Fortmann2015}, see (\ref{ex:disjunctspelling1}).
...
While the relation in N+N compounds is quite varied as well as unspecified and unspecific (see Section~\ref{sub:nncompounds}), there are only two possible relations within N+V units, and these relations are determined by the verb's argument structure.
It's either an argument relation or an oblique relation where all distributional restrictions apply that would apply in a syntactic realisation of the same verb.
As a consequence, there is always a syntactic paraphrase for N+V units with an oblique relation where the noun occurs in a prepositional phrase which is an adjunct to the verb.%
...
The fact that the relation can be decoded for almost all N+V units means that the morphological construction marked by graphemic univerbation remains in competition with a syntactic construction with distinct syntactic words separated by spaces in writing.
...
This is true although univerbation of N+V units with an argument relation dates back to Middle High German (\textit{lobpreisen} `praise', literally `to praise compliment') and even Old High German (\textit{hals-werf\=on} `turning around', literally `to throw neck'),\roland{Correct gloss?} see \citet[517]{Wurzel1994}, \citet[334]{Wurzel1998}.
For N+V units with an oblique relation, \citet[89]{Morcinek2013} notices that dictionaries from between 1750 and 1993 AD list novel N+V units with an oblique relation with increasing frequency.
...
We still have to show why and under which conditions we assume true compounding and (including potential V-to-N conversion of the head and subsequent graphemic univerbation) to be preferred.
As already mentioned, the morphological relaisation of N+V units is a type of noun incorporation.
N+V units are usually seen as the only cases of potential incorporation in Modern German \parencite[245]{Eisenberg2020a}, which is why we argued above that argument and oblique relations are prototypically realised syntactically in the form of objects and adjuncts.
...
Semantically, the both cases (syntactic and morphological combination) have the same properties which are typical of incorporation.}
...
In contrast with the factors that prevent univerbation and make N+V units reluctabt compounds, this is a factor that we assume to facilitate their univerbation.

As a clear indicator of full conversion of the verb to a noun and thus a clear indicator of compounding is the presence of a linking element in between the two parts of the N+V unit, such as in \textit{Leistungsschießen} `(high) performance shooting\slash competitive shooting' (or \textit{leistungsgeschossen} in the participle, etc.).
Such linking elements are otherwise exclusive to N+N compounds (see Section~\ref{sub:nncompounds}) and should thus indicate full nominalisation of V.
The fact that such linking elements also occur in clearly verbal forms of N+V units (such as \textit{leistungsgeschossen}) is perplexing.
These cases are rare, however, and we suspect that they are the result of some type of back-formation, where a fully nominalised N+V unit is re-analysed as a verbal N+V unit.
The infrequency of the phenomenon makes it hard to assess in the present study, and we do not go into more detail.
...
As argued, N+V units with an oblique relation stand in weaker competition with a syntactic realisation compared with those that have an argument relation.
...
The presence of a linking element cannot be conceived of as a causal influencing factor, but rather an additional indicator of compound status (see Section~\ref{sub:nvunitsasreluctantcompounds}.
Since linking elements are not paradigmatic in N+V units realised syntactically, its presence is expected in cases where the unit is used as a true N+N compound.
...
As a preliminary step, the basis for any empirical look at N+V units and their spelling has to be a data-driven assessment of which such units exist, how strongly they alternate, and what their item-specific tendencies are, \ie how clearly they tend to be spelled as one word or two words.

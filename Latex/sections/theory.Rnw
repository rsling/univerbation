...
\subsection{Probabilisic grammar and graphemic evidence}
\label{sub:probabilisticgrammarandgraphemicevidence}
...
\roland{Use ``oblique semantic relation'' (Gaeta/Zeldes 2018:20) for adverbial relation.}
...
For an N+V unit to undergo graphemic univerbation (\ie, a downgrading of a syntactic to a morphological construction in the sense of \citealt[2]{Lehmann2021}) systematically, it must resemble established morphological constructions enough to be classified as an instance of that construction itself.%
...
The prototypical and by far most producitve morphological construction combining more than one stem is noun + noun compounding (N+N), to which we turn now in some detail.%
\footnote{Adjectives also enter compounds as the head, such as in \textit{feuerrot} `red like fire', literally `fire red'.
However, they are much less frequent than N+N compounds, and we consequently don't discuss them here.
...
\footnote{\citet[130]{Nuebling2017} finds that the prenominal genitive begins to give way to the postnominal genitive in the 13th century.
...
The N+N compound construction is semantically at least as unspecific as the syntactic genitive construction to which it is diachronically related (see also \citealt[239]{Eisenberg2020a}).
...
Diachronically, linking elements stem from diverse sources, but the overall pattern of inserting them is related to the former morphological marking in prenominal genitives \parencite[55--57]{Nuebling2017}.
...
As opposed to compounding with nominal heads (as discussed in the previous section), compounding with verbal heads is not a highly productive pattern in German.%
\footnote{\citet{Günther1997} counts roughly 400 lexicalised N+V compounds in \citet{Muthmann1988} (see also \citealt[245]{Eisenberg2020a}).}
...
There can be intervening material in between the noun and the verb in some contexts (the infinitival particle \textit{zu} and the inflecional affix \textit{ge-} of past participles).
Additionally, the noun and the verb can be separated and rearranged as in verb-second order, where N+V units resemble particle verbs \parencite[603]{Fortmann2015}.
...
While the relation in N+N compounds is vastly unspecific (see Section~\ref{sub:nncompounds}), there are only two possible relations within N+V units, and these relations are determined by---and above all decodable through---the distributional properties of the verb (including its argument structure).
...
The arguments of a verb as well as its adjuncts are usually realised as syntactic dependants of the verb.
...
This competition between a morphological construction and a syntactic construction was pointed out with varying terminology by---among others---\citet[12]{FleischerBartz2012}, \citet[13]{Schluecker2012}, and \citet[88]{Morcinek2013}.\ulrike{On your p.~5, you mention homonymous morphological and syntactic constructions, not giving examples. I fail to see where \textbf{homonoymous} constructions exist. It seems to be the point that in N+V units, N looses its genuine syntactic properties (article, modifiability, \ldots). The constructions are therefore never homonymous. --- haben wir schon drüber gesprochen: synchron zeigen sich diese Formen sehr wohl als homonym. Interessant ist ja hier, inwiefern gerade Countnouns in Kontexten verwendet werden, die letztlich denen von Massnouns ähneln und so als Varianten zueinander stehen, Univerbierung begünstigt.
Hier vor allem auch Verweis auf Hopper/Thomson (1984: 713-717) und Vogel (2000:472)}
...
Functionally speaking, the construction exists in order to express the new event concept which requires the generic\slash unspecific reading of the noun.
...
This is why we call them reluctant compounds.

\roland{Mention nominal\slash verbal contexts.}

\roland{Mention linking elements!}
...
What favors univerbation? Greater similarity to N+N prototype and reduced competition from syntactic realisation. Hence:\roland{We only look at cases where the alternation occurs, \ie, contact position.}
...
(3) Semantically weak/generic verbs like haben, machen, fahren, \ldots because they only denote a specific concept together with the noun.

(4) Idiosyncratic diachronic status. Each N+V unit may have undergone conventionaisation\slash lexicalisation to different degrees. Tendencies are therefore likely to vary across individual N+V units.

The individual diachronic tendencies as reflected in Modern German are measured in the corpus (=degree of conventionalisation\slash lexicalisation) and confirmed in the experiment (=individual grammar).

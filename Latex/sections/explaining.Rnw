...
Another point that needs further examination in the future is the productivity of the N+V construction.
...
Related to the question of productivity, we might ask whether some of the N+V units are actually the result of backformation processes.
For example, the verb \textit{zwangsernähren} `to force feed' is likely a backformation of the N+N compound \textit{Zwangsernährung} `force feeding' (with \textit{Ernährung} `feeding' being derived from \textit{ernähren} `to feed'), and it now appears as a N+V unit with the full array of finite and infinite verb forms.
It is difficult to quantify the effect of such backformations on the present study.
...
This might be yet another indication that we are dealing with backformation processes in some of these cases.

...
<<verbtendencies, results="asis", fig.showtext=TRUE, fig.pos="htbp", fig.height=4, fig.cap="Attraction scores for N+V units with four different lexical verbs (\\textit{machen} `to make\\slash do', \\textit{laufen} `to run\\slash walk', \\textit{schießen} `to shoot', \\textit{springen} `to jump'); $n$ is the number of N+V units with the respective V head">>=
...
Intuitively and from looking at the data, it appears that the units with an oblique relation are formed much more productively compared to the ones with an oblique relation.
If the ones with an oblique relation are formed less productively, they should be more strongly lexicalised, which might be a reason for their stronger tendency to univerbate.

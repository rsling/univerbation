...
Another model was specified in Section~\ref{sub:resultsexperiment} as (\ref{eq:expglmmformula}) and repeated here as (\ref{eq:expglmmformula2}).
...
Since the models are parallel in structure, we provide a more precise specification here only for (\ref{eq:corpusglmmr2}).
